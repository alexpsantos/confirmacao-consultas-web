...
export function AdminPatients({ data, session, reload }: CommonProps & { data: Patient[] }) {
  ...
  const filtered = useMemo(() => data.filter(item => {
    ...
    : sort === 'status' ? Number(b.active) - Number(a.active)
      : a.fullName.localeCompare(b.fullName, 'pt-BR')), [data, search, professional, status, sort])
  const pages = Math.max(1, Math.ceil(filtered.length / pageSize)), safePage = Math.min(page, pages - 1)
  const visible = filtered.slice(safePage * pageSize, (safePage + 1) * pageSize)
  const hasFilters = Boolean(search || professional || status || sort !== 'patient')
  function change(setter: (value: string) => void, value: string) { setter(value); setPage(0) }
  function clearFilters() { setSearch(''); setProfessional(''); setStatus(''); setSort('patient'); setPage(0) }
  ...
  if (selected) return <section className="admin-professional-detail">
    ...
    {error && <div className="error banner">{error}</div>}
    <div className="admin-professional-data"><h3>Dados cadastrais</h3><dl><div><dt>Nome completo</dt><dd>{selected.fullName}</dd></div><div><dt>E-mail</dt><dd>{selected.email || '—'}</dd></div><div><dt>Telefone</dt><dd>{formatPhone(selected.phone)}</dd></div><div><dt>Data de nascimento</dt><dd>{selected.birthDate ? new Intl.DateTimeFormat('pt-BR', {timeZone:'UTC'}).format(new Date(`${selected.birthDate}T00:00:00Z`)) : '—'}</dd></div><div><dt>Canal preferencial</dt><dd>{selected.preferredChannel === 'WHATSAPP' ? 'WhatsApp' : 'E-mail'}</dd></div><div><dt>Lembretes pelo WhatsApp</dt><dd>{selected.whatsappRemindersEnabled === false ? 'Desativados' : 'Ativados'}</dd></div><div><dt>Profissional responsável</dt><dd>{selected.professionalName} · {selected.professionalEmail}</dd></div></dl></div>
  ...
  return <section className="panel"><div className="panel-title"><div><h2>{filtered.length} {filtered.length === 1 ? 'paciente encontrado' : 'pacientes encontrados'}</h2><p>Consulte e gerencie os pacientes da plataforma.</p></div></div>
    ...
    <table><thead><tr><th>Paciente</th><th>Profissional responsável</th><th>Telefone</th><th>Status</th><th /></tr></thead>
      <tbody>{visible.map(item => <tr key={item.id} className={`admin-professional-row ${item.active ? '' : 'inactive-row'}`} onClick={() => setSelectedId(item.id)}><td><button className="entity-link" onClick={() => setSelectedId(item.id)}>{item.fullName}</button><small>{item.email}</small></td>
        <td><strong>{item.professionalName}</strong><small>{item.professionalEmail}</small>{item.professionalRegistrationNumber && <small>Registro: {item.professionalRegistrationNumber}</small>}</td><td>{formatPhone(item.phone)}{!validPhone(item.phone) && <small className="data-warning">⚠ Telefone inválido</small>}</td>
