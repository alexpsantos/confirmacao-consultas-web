...
export function layoutAppointments(items:Appointment[]){const result:{item:Appointment;column:number;columns:number}[]=[];let group:Appointment[]=[],groupEnd=-Infinity;const flush=()=>{const ends:number[]=[];const positioned=group.map(item=>{const start=new Date(item.startsAt).getTime(),end=new Date(item.endsAt).getTime();let column=ends.findIndex(value=>value<=start);if(column<0){column=ends.length;ends.push(end)}else ends[column]=end;return{item,column}});result.push(...positioned.map(entry=>({...entry,columns:ends.length})));group=[];groupEnd=-Infinity};for(const item of items.slice().sort((a,b)=>new Date(a.startsAt).getTime()-new Date(b.startsAt).getTime())){const start=new Date(item.startsAt).getTime(),end=new Date(item.endsAt).getTime();if(group.length&&start>=groupEnd)flush();group.push(item);groupEnd=Math.max(groupEnd,end)}flush();return result}

export function SessionsView({session,patients,professional,initialPatientId,openNewSession,onInitialPatientHandled}:{session:Session;patients:Patient[];professional?:Professional;initialPatientId?:string;openNewSession?:boolean;onInitialPatientHandled?:()=>void}){
  ...
  useEffect(()=>{if(initialPatientId||openNewSession){setDraftPatientId(initialPatientId);openNext();onInitialPatientHandled?.()}},[initialPatientId,openNewSession,onInitialPatientHandled]);
      ...
      <div className="calendar-scroll" ref={calendarRef}>
        ...
        <div className="calendar-body">
          ...
          <div className="calendar-days">{days.map(day=>{
            const current=day.toDateString()===new Date().toDateString(),dayItems=layoutAppointments(items.filter(item=>new Date(item.startsAt).toDateString()===day.toDateString()));
            return <div className={`calendar-day ${current?'today':''}`} key={day.toISOString()} style={{height:(END_HOUR-START_HOUR)*HOUR_HEIGHT}}>
              ...
              {dayItems.map(({item,column,columns})=>{const start=new Date(item.startsAt),finish=new Date(item.endsAt),startMinutes=start.getHours()*60+start.getMinutes(),endMinutes=finish.getHours()*60+finish.getMinutes(),top=Math.max(0,(startMinutes-START_HOUR*60)/60*HOUR_HEIGHT),height=Math.max(30,(endMinutes-startMinutes)/60*HOUR_HEIGHT),tooltip=item.notes?`Observação: ${item.notes}`:undefined;return <button key={item.id} title={tooltip} style={{top,height,left:`calc(4px + ${column} * ((100% - 8px) / ${columns}))`,width:`calc((100% - 8px) / ${columns})`,right:'auto'}} className={`session-card ${item.status.toLowerCase()}`} onClick={()=>setEditing(item)}><time>{start.toLocaleTimeString('pt-BR',{hour:'2-digit',minute:'2-digit'})}–{finish.toLocaleTimeString('pt-BR',{hour:'2-digit',minute:'2-digit'})}{item.notes&&<span className="note-indicator" aria-label="Possui observação"> ●</span>}</time><strong>{item.patientName}</strong><small>{statusLabels[item.status]} · {item.modality==='ONLINE'?'Online':'Presencial'}</small></button>})}
            </div>})}</div>
        </div>
      </div>
    </section>
    {(editing||draft)&&<SessionModal item={editing} initial={draft} patientId={draftPatientId} patients={patients.filter(p=>p.active)} defaultDuration={professional?.defaultSessionMinutes??60} defaultModality={professional?.defaultModality??'PRESENTIAL'} close={()=>{setEditing(null);setDraft(null);setDraftPatientId(undefined)}} save={async data=>{if(editing)await api.updateSession(session,editing.id,data);else await api.createSession(session,data);setEditing(null);setDraft(null);setDraftPatientId(undefined);await load()}} cancel={editing?async()=>{await api.updateSession(session,editing.id,{patientId:editing.patientId,startsAt:editing.startsAt,endsAt:editing.endsAt,modality:editing.modality,status:'CANCELED',meetingLink:editing.meetingLink,notes:editing.notes});setEditing(null);await load()}:undefined}/>}
  </>
}

function SessionModal({item,initial,patientId,patients,defaultDuration,defaultModality,close,save,cancel}:{item:Appointment|null;initial:Date|null;patientId?:string;patients:Patient[];defaultDuration:number;defaultModality:'PRESENTIAL'|'ONLINE';close:()=>void;save:(data:object)=>Promise<void>;cancel?:()=>Promise<void>}){
  ...
  return <div className="modal-backdrop"><section className="modal"><div className="modal-head"><h2>{item?'Editar sessão':'Nova sessão'}</h2><button onClick={close}>×</button></div><form className="form-grid" onSubmit={submit}><label className="span">Paciente<select name="patientId" defaultValue={item?.patientId??patientId} required><option value="">Selecione</option>{patients.map(p=><option key={p.id} value={p.id}>{p.fullName}</option>)}</select></label><label>Início<input name="startsAt" type="datetime-local" min={item?undefined:localValue(new Date())} defaultValue={localValue(start)} required/></label><label>Término<input name="endsAt" type="datetime-local" defaultValue={localValue(finish)} required/></label><label>Modalidade<select name="modality" defaultValue={item?.modality??defaultModality}><option value="PRESENTIAL">Presencial</option><option value="ONLINE">Online</option></select></label>{item?<label>Status<select name="status" defaultValue={item.status}><option value="SCHEDULED">Agendada</option><option value="CONFIRMED">Confirmada</option></select></label>:<input type="hidden" name="status" value="SCHEDULED"/>}<label className="span">Link da sessão (opcional)<input name="meetingLink" type="url" defaultValue={item?.meetingLink??''} maxLength={500}/></label><label className="span">Observação (opcional)<input name="notes" defaultValue={item?.notes??''} maxLength={500}/></label>{error&&<div className="error span">{error}</div>}{item&&cancel&&<div className="span"><button type="button" className="link danger-link" disabled={busy} onClick={async()=>{if(!window.confirm(`Cancelar a sessão de ${item.patientName}?`))return;setBusy(true);setError('');try{await cancel()}catch(e){setError(e instanceof Error?e.message:'Não foi possível cancelar a sessão');setBusy(false)}}}>Cancelar sessão</button></div>}<div className="form-actions span"><button type="button" className="ghost" onClick={close}>Fechar sem salvar</button><button className="primary" disabled={busy}>{busy?'Salvando…':'Salvar sessão'}</button></div></form></section></div>
}
